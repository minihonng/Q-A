...
$(function () {
    ...
    var index = 0;
    $('.rotate_btn').click(function (event) {
        index++;
        var _this = $(this);
        if (!_this.hasClass('act')) {
            !_this.addClass('act');
            methods.star_animate.call(this);
            // $('.rotate_box dd').rotate(methods.getDataIndex(prizeList1));
            if (index < 3) {
                $('.rotate_box dd').rotate(methods.getRandom(4, index));
            }
        }
    })
    ...
    var methods = {
        ...
        getRandom: function (num, indexs) {
            var arr = [];
            _num = 3;
            var val = Math.floor(Math.random() * (num - 2) + 2);
            (indexs == 1) ? val = 0: (indexs == 2 ? val = 1 : val);
            do {
                arr.push(val);
                _num--
            }
        ...
        //             if (prizeList[i]['prizeid'] === prizeMsg[i]['prizeid']) {
        //                 _index = i;
        //             }
        //         })
        //     }
